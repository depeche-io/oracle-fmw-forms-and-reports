...
REVOKE create any index FROM INFRADOMAIN_WLS
/

REVOKE create any trigger FROM INFRADOMAIN_WLS
/

REVOKE create any table FROM INFRADOMAIN_WLS
/

REVOKE create any view FROM INFRADOMAIN_WLS
/

REVOKE create any index FROM INFRADOMAIN_WLS_RUNTIME
/

REVOKE create any trigger FROM INFRADOMAIN_WLS_RUNTIME
/

GRANT create trigger to INFRADOMAIN_WLS
/

GRANT create table to INFRADOMAIN_WLS
/

GRANT create view to INFRADOMAIN_WLS
/

GRANT create trigger to INFRADOMAIN_WLS_RUNTIME
/
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'UPDATE '||loc||'.SCHEMA_VERSION_REGISTRY SET status=''VALID'', modified=CURRENT_TIMESTAMP WHERE comp_id=''WLS'' AND mrc_name=''INFRADOMAIN''';
END;
...
ALTER SESSION SET CURRENT_SCHEMA=INFRADOMAIN_IAU_VIEWER
/
...
BEGIN
  SELECT owner into loc from all_tables where table_name = 'SCHEMA_VERSION_REGISTRY$' and owner in ('SYSTEM', 'FMWREGISTRY');
  EXECUTE IMMEDIATE 'UPDATE '||loc||'.SCHEMA_VERSION_REGISTRY SET status=''VALID'', modified=CURRENT_TIMESTAMP WHERE comp_id=''IAU'' AND mrc_name=''INFRADOMAIN''';
END;
