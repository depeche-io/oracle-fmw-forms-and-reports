...
DECLARE
  ...
  pragma exception_init(insuff_privs, -01031);
  user_conflict exception;
  pragma exception_init(user_conflict, -01920);
BEGIN
  execute immediate 'CREATE TABLE SYSTEM.SCHEMA_VERSION_REGISTRY$ (comp_id VARCHAR2(30) NOT NULL,  comp_name VARCHAR2(255),  mrc_name VARCHAR2(30) NOT NULL,  mr_name VARCHAR2(30),  mr_type VARCHAR2(30),  owner VARCHAR2(30),  version VARCHAR2(30),  status VARCHAR2(11),  custom1 NUMBER,  custom2 VARCHAR2(30),  edition VARCHAR2(30),  upgraded CHAR(1) DEFAULT ''N'',  start_time TIMESTAMP,  modified TIMESTAMP)';
exception
  when insuff_privs then
  BEGIN
    execute immediate 'CREATE USER FMWREGISTRY IDENTIFIED BY "ZHjOjrjnP0apF8uz1C5DyS86yxL6SL" QUOTA UNLIMITED ON DATA ACCOUNT LOCK';
  EXCEPTION WHEN user_conflict THEN NULL;
  END;
    execute immediate 'CREATE TABLE FMWREGISTRY.SCHEMA_VERSION_REGISTRY$ (comp_id VARCHAR2(30) NOT NULL,  comp_name VARCHAR2(255),  mrc_name VARCHAR2(30) NOT NULL,  mr_name VARCHAR2(30),  mr_type VARCHAR2(30),  owner VARCHAR2(30),  version VARCHAR2(30),  status VARCHAR2(11),  custom1 NUMBER,  custom2 VARCHAR2(30),  edition VARCHAR2(30),  upgraded CHAR(1) DEFAULT ''N'',  start_time TIMESTAMP,  modified TIMESTAMP)';
END;
/
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW '||loc||'.SCHEMA_VERSION_REGISTRY AS SELECT comp_id,  comp_name,  mrc_name,  mr_name,  mr_type,  owner,  version,  status,  upgraded,  start_time,  modified,  edition  FROM '||loc||'.SCHEMA_VERSION_REGISTRY$ ORDER BY comp_id';
END;
...
BEGIN
  SELECT owner into loc from all_tables where table_name = 'SCHEMA_VERSION_REGISTRY$' and owner in ('SYSTEM', 'FMWREGISTRY');
  EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM SCHEMA_VERSION_REGISTRY FOR '||loc||'.SCHEMA_VERSION_REGISTRY';
END;
...
CREATE  TABLESPACE "INFRADOMAIN_IAS_OPSS" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_ias_opss.dbf' SIZE 60M REUSE  AUTOEXTEND ON  MAXSIZE  UNLIMITED  ;

CREATE  TEMPORARY  TABLESPACE "INFRADOMAIN_IAS_TEMP" EXTENT MANAGEMENT LOCAL  TEMPFILE '+DATA/INFRADOMAIN_iastemp.dbf' SIZE 100M REUSE  AUTOEXTEND ON  NEXT 50M MAXSIZE  UNLIMITED  ;

CREATE  TABLESPACE "INFRADOMAIN_IAS_UMS" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_UMS.dbf' SIZE 100M REUSE  AUTOEXTEND ON  NEXT 30M MAXSIZE  UNLIMITED  ;

CREATE  TABLESPACE "INFRADOMAIN_IAU" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_iau.dbf' SIZE 60M REUSE  AUTOEXTEND ON  MAXSIZE  UNLIMITED  ;

CREATE  TABLESPACE "INFRADOMAIN_MDS" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_mds.dbf' SIZE 100M REUSE  AUTOEXTEND ON  NEXT 50M MAXSIZE  UNLIMITED  ;

CREATE  TABLESPACE "INFRADOMAIN_STB" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_svctbl.dbf' SIZE 10M REUSE  AUTOEXTEND ON  NEXT 2M MAXSIZE  UNLIMITED  ;

CREATE  TABLESPACE "INFRADOMAIN_WLS" EXTENT MANAGEMENT LOCAL  AUTOALLOCATE  SEGMENT SPACE MANAGEMENT  AUTO  DATAFILE '+DATA/INFRADOMAIN_wlsservices.dbf' SIZE 60M REUSE  AUTOEXTEND ON  NEXT 10M MAXSIZE  UNLIMITED  ;
...
BEGIN
  ...
  BEGIN
    SELECT COUNT(*) INTO compCnt FROM SCHEMA_VERSION_REGISTRY
      WHERE MRC_NAME='INFRADOMAIN' and COMP_ID='WLS';
    IF compCnt = 0 THEN
      EXECUTE IMMEDIATE 'INSERT INTO '||loc||'.SCHEMA_VERSION_REGISTRY$ (comp_id, comp_name, mrc_name, mr_name, mr_type, owner, version, status, edition, custom1) VALUES (''WLS'', ''WebLogic Services'', ''INFRADOMAIN'', ''WLS'', ''WLS'', ''INFRADOMAIN_WLS'', ''12.2.1.0.0'', ''LOADING'', '''', 0)';
    END IF;
  END;
END;
...
accept SCHEMA_PASSWORD char prompt "Enter the schema password for user INFRADOMAIN_WLS : " hide;
begin
 execute immediate 'CREATE USER INFRADOMAIN_WLS IDENTIFIED BY &SCHEMA_PASSWORD DEFAULT TABLESPACE INFRADOMAIN_WLS TEMPORARY TABLESPACE INFRADOMAIN_IAS_TEMP'; 
end;

/

GRANT connect TO INFRADOMAIN_WLS
/

GRANT create type TO INFRADOMAIN_WLS
/

GRANT create procedure TO INFRADOMAIN_WLS
/

GRANT create table TO INFRADOMAIN_WLS
/

GRANT create sequence TO INFRADOMAIN_WLS
/

GRANT create any index to INFRADOMAIN_WLS
/

GRANT create any trigger to INFRADOMAIN_WLS
/

GRANT create any table to INFRADOMAIN_WLS
/

GRANT create any view to INFRADOMAIN_WLS
/

ALTER USER INFRADOMAIN_WLS QUOTA unlimited ON INFRADOMAIN_WLS
/

accept ADDITIONAL_SCHEMA_PASSWORD1 char prompt "Enter the schema password for user INFRADOMAIN_WLS_RUNTIME : " hide;
begin
 execute immediate 'CREATE USER INFRADOMAIN_WLS_RUNTIME IDENTIFIED BY &ADDITIONAL_SCHEMA_PASSWORD1 DEFAULT TABLESPACE INFRADOMAIN_WLS TEMPORARY TABLESPACE INFRADOMAIN_IAS_TEMP'; 
end;

/

GRANT connect TO INFRADOMAIN_WLS_RUNTIME
/

GRANT create type TO INFRADOMAIN_WLS_RUNTIME
/

GRANT create procedure TO INFRADOMAIN_WLS_RUNTIME
/

GRANT create table TO INFRADOMAIN_WLS_RUNTIME
/

GRANT create sequence TO INFRADOMAIN_WLS_RUNTIME
/

GRANT create any index to INFRADOMAIN_WLS_RUNTIME
/

GRANT create any trigger to INFRADOMAIN_WLS_RUNTIME
/

ALTER USER INFRADOMAIN_WLS_RUNTIME QUOTA unlimited ON INFRADOMAIN_WLS
/
...
BEGIN
  SELECT owner into loc from all_tables where table_name = 'SCHEMA_VERSION_REGISTRY$' and owner in ('SYSTEM', 'FMWREGISTRY');
  EXECUTE IMMEDIATE 'UPDATE '||loc||'.SCHEMA_VERSION_REGISTRY$ SET status=''LOADED'' WHERE comp_id=''WLS'' AND mrc_name=''INFRADOMAIN''';
END;
...
GRANT REGISTRYACCESS TO INFRADOMAIN_WLS
/
